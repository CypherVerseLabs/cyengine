...
import { CypherverseSignaller } from "./signallers/CypherverseSignaller";
...
import { getCypherverseIceServers } from "./ice";
...
export const useConnection = (
  ...
  const connect = async (config?: ConnectionConfig) => {
    ...
    if (!finalConfig.iceServers) {
      const servers = await getCypherverseIceServers(finalConfig.host);
    ...
    p.on("open", async () => {
      ...
      const s =
        isLocalNetwork() && !finalConfig.host
          ...
          : new CypherverseSignaller(p, finalConfig);
